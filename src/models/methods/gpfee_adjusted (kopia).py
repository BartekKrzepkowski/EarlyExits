...
class GPFEE(torch.nn.Module):
    ...
    def concatenation(self, states, current_layer):
        ...
        return past_state, future_state, prev_gate
    ...
    def run_train(self, x_true, y_true):
        ...
        for i in range(self.n_layers-1):
            ...
            state_pi, state_fi, prev_gate = self.inc_strategy(past_states[:i+1] + future_states, i)
        ...
        if not self.is_model_frozen:
            output_main_head, _, _ = self.inc_strategy(past_states, self.n_layers-1)
    ...
    def run_val(self, x_true, y_true):
        ...
        for i in range(self.n_layers-1):
            ...
            state_pi, state_fi, prev_gate = self.inc_strategy(past_states + future_states, i)

            balance = torch.sigmoid(self.adaptive_balance(prev_gate))
            z_i = balance * state_pi + (1 - balance) * state_fi
        ...
        if not (exit_mask_local).all():
            ...
            state_main_head, output_main_head = fg.send(repr_i)
            if not self.is_model_frozen:
                output_main_head, _, _ = self.inc_strategy(past_states + [state_main_head])
            ...
            for j, k in enumerate(exit_indices_global):
                self.sample_outputs[k] = output_main_head[j]
    ...
    def find_exit(self, z_i, head_idx):
        p_i = F.softmax(z_i, dim=1)
        head_confidences_i = self.entropy(p_i)
