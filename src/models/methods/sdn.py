...
class SDN(torch.nn.Module):
    def __init__(self, model, layers_dim, num_classes, config_ee):
        ...
        self.reduction = torch.nn.ModuleList([torch.nn.Linear(hidden_dim, num_classes) for _ in layers_dim])

    def concatenation(self, states, current_layer):
        state = states[current_layer]
        state = self.reduction[current_layer](state)
        return state
