...
from src.common.utils import adjust_evaluators, adjust_evaluators_pre_log, create_paths, save_model
...
class TrainerClassification:
    ...
    def run_epoch(self, phase, config):
        ...
        progress_bar = tqdm(self.loaders[phase], desc=f'run_epoch: {phase}', mininterval=30,
                            leave=True, total=loader_size, position=0, ncols=50) # ruszałem
            ...
            if 'train' in phase:
                ...
                if (i + 1) % config.grad_accum_steps == 0 or (i + 1) == loader_size:
                    if config.whether_clip:
                        torch.nn.utils.clip_grad_norm(filter(lambda p: p.requires_grad, self.model.parameters()),
                                                      config.clip_value)
                    self.optim.step()
    ...
    def update_assets(self, assets_target: Dict, assets_source: Dict, denom, scope, phase: str):
        '''
        Update epoch assets
        Args:
